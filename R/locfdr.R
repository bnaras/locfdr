...
#'   simultaneous consideration.  The calculations assume a large number
#'   of cases, say `length(zz)` exceeding 200.  Results may be improved
#'   by transforming zz so that its elements are theoretically distributed
#'   as \eqn{N(0,1)} under the null hypothesis.  See the locfdr vignette
#'   for tips on creating zz.
#' @param bre Number of breaks in the discretization of the z-score axis,
...
#' @param df Degrees of freedom for fitting the estimated density \eqn{f(z)}.
#' @param pct Excluded tail proportions of zz's when fitting \eqn{f(z)}.
#'   `pct=0` includes full range of zz's.  pct can also be a 2-vector,
#'   describing the fitting range.
#' @param pct0 Proportion of the zz distribution used in fitting the null
#'   density \eqn{f_0(z)} by central matching.  If a 2-vector, e.g.
#'   `pct0=c(0.25,0.60)`, the range `[pct0[1], pct0[2]]` is used.  If
#'   a scalar, `[pct0, 1-pct0]` is used.
#' @param nulltype Type of null hypothesis assumed in estimating
#'   \eqn{f_0(z)}, for use in the fdr calculations.  0 is the theoretical
#'   null \eqn{N(0,1)}, 1 is maximum likelihood estimation, 2 is central
#'   matching estimation, 3 is a split normal version of 2.
#' @param type Type of fitting used for \eqn{f}; 0 is a natural spline,
#'   1 is a polynomial, in either case with degrees of freedom df (so
#'   total degrees of freedom including the intercept is `df+1`).
#' @param plot Plots desired.  0 gives no plots. 1 gives single plot
#'   showing the histogram of zz and fitted densities \eqn{f} and
#'   \eqn{p_0 f_0}.  2 also gives plot of fdr, and the right and left
#'   tail area Fdr curves.  3 gives instead the \eqn{f_1} cdf of the
#'   estimated fdr curve; plot=4 gives all three plots.
#' @param mult Optional scalar multiple (or vector of multiples) of the
#'   sample size for calculation of the corresponding hypothetical Efdr
#'   value(s).
#' @param mlests Optional vector of initial values for
#'   \eqn{(\delta_0, \sigma_0)} in the maximum likelihood iteration.
#' @param main Main heading for the histogram plot when `plot>0`.
#' @param sw Determines the type of output desired.  2 gives a list
#'   consisting of the last 5 values listed under Value below.  3 gives
#'   the square matrix of dimension bre-1 representing the influence
#'   function of log(fdr).  Any other value of sw returns a list
#'   consisting of the first 7 (8 if mult is supplied) values listed below.
#'
...
#'     \item{fp0}{a matrix of estimated parameters delta, sigma, and p0,
#'       along with their standard errors, for all three null types.}
#'     \item{Efdr}{the expected false discovery rate for the non-null
#'       cases.}
#'     \item{cdf1}{a 99x2 matrix giving the estimated cdf of fdr under
#'       the non-null distribution \eqn{f_1}.}
...
#'   choice of a null hypothesis", \emph{Jour Amer Stat Assoc}, \bold{99},
#'   pp. 96--104.
#'
#'   Efron, B. (2006) "Size, Power, and False Discovery Rates".
#'
#'   Efron, B. (2007) "Correlation and Large-Scale Simultaneous
#'   Significance Testing", \emph{Jour Amer Stat Assoc}, \bold{102},
#'   pp. 93--103.
#'
